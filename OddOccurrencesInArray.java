...
// Test result: https://app.codility.com/demo/results/training2KDU5Z-R6F/
// Test score: 100%
// Time complexity: O(N)
...
class Solution {
    public int solution(int[] A) {
        Map<Integer, Integer> pairedValues = new HashMap<Integer, Integer>();
        for (int i = 0; i < A.length; i++) {
            if (pairedValues.containsKey(A[i])) {
                pairedValues.remove(A[i]);
            } else {
                pairedValues.put(A[i], 1);   
            }
        }
        
        return pairedValues.keySet().iterator().next();
    }
}
